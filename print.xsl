...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="vat-rate"><xsl:value-of select="winelist/vatrate"/></xsl:variable>
		...
		<fo:root>
			...
			<fo:page-sequence master-reference="A4"
				...
				<fo:flow flow-name="xsl-region-body">
					...
					<xsl:apply-templates select="style[@id='white'] break="page">
						<xsl:with-param name="header-text">
							Best Buys wines for under £<xsl:value-of select="$price-break"/> a bottle
						</xsl:with-param>
						<xsl:with-param name="price-op" select="'under'/>
					</xsl:apply-templates>

					<xsl:apply-templates select="style[@id='rosé']"/>

					<xsl:apply-templates select="style[@id='red']" break="page">
						<xsl:with-param name="price-op" select="'under'/>
					</xsl:apply-templates>

					<xsl:apply-templates select="style[@id='sparkling']" break="page">
						<xsl:with-param name="header-text">
							Champagne &amp; sparkling
						</xsl:with-param>
	...
	<xsl:template match="signature">
		<fo:block margin-bottom="12pt" text-align="right">
	...
	<xsl:template match="@break">
		<xsl:attribute name="break-before"><xsl:value-of select="."/></xsl:attribute>
	</xsl:template>
	...
	<xsl:template match="wine">
		...
		<xsl:param name="show-case-price"   select="format-number(0.05*round(($case-plus-vat div  1) div 0.05),'##0.00')"/>
		<xsl:param name="show-bottle-price" select="format-number(0.05*round(($case-plus-vat div 12) div 0.05),'##0.00')"/>
		<xsl:param name="show-magnum-price" select="format-number(0.05*round(($case-plus-vat div  6) div 0.05),'##0.00')"/>

		<xsl:choose>
			<xsl:when test="$price-op='under'">
	...
	<xsl:template name="show-this-wine">
		...
		>
			...
			<fo:leader leader-pattern="space"/>
			<xsl:apply-templates select="caseprice"><xsl:with-param name="show_case-price"/></xsl:apply-templates>
		</fo:block>
